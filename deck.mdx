...
<Notes>
  - This makes sense, right? The light can't be green and red at the same time.
</Notes>
...
<Notes>
  - In my contrived example, the event is me pressing the "next" button or the button to
  cut or restore the signal, but in real life these events would probably be timers
  running out, or a "signal lost" or "signal restored" event for the flashing red state
</Notes>
